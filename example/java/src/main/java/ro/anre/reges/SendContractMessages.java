...
public class SendContractMessages {
    ...
    @Value("${props.in-contract-folder-name}")
        ...
        if(this.checkRequirements(inPath, outPath)){
            ...
            if (files != null) {
                ...
                for (File file: files) {
                    ...
                    mapper.writeValue(new File(outPath + "response-con-" + resp.getHeader().getMessageId() +".xml"), resp);
...
//        ResponseEntity<String> result = restTemplate.postForEntity(uri, httpEntity, String.class);
        ...
        String url = SpringBootConsoleApplication.apiDomain + "/api/Contract";
